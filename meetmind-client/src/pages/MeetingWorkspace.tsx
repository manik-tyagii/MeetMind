...
export const MeetingWorkspace: React.FC = () => {
  ...
  const createMutation = useMutation({
    ...
    onError: (err: any) => {
      ...
      const msg = err?.response?.data?.message || "Failed to create meeting note";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const updateMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      const status = err?.response?.status;
      const msg = err?.response?.data?.message || "Failed to save meeting notes";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      const status = err?.response?.status;
      const msg = err?.response?.data?.message || "Failed to delete meeting note";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const summaryMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      const status = err?.response?.status;
      const msg = err?.response?.data?.message || "Failed to compile summary via Gemini";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const emailMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      const status = err?.response?.status;
      const msg = err?.response?.data?.message || "Failed to draft follow-up email via Gemini";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const tasksMutation = useMutation({
    ...
    onError: (err: any) => {
      console.error(err);
      const status = err?.response?.status;
      const msg = err?.response?.data?.message || "Failed to generate tasks via Gemini";
      setFormError(`${status ? '[' + status + '] ' : ''}${msg}`);
      addToast(`${status ? '[' + status + '] ' : ''}${msg}`, "error");
    }
  });
  ...
  const handleSave = () => {
    if (!title.trim()) {
      const msg = "Meeting title is required!";
      setFormError(msg);
            ...
            <GlassCard className="flex-1 min-h-0 flex flex-col p-4 sm:p-6 overflow-hidden">
              {activeTab === 'editor' ? (
                <div className="flex-1 flex flex-col space-y-4 overflow-y-auto">
                  {/* Title input */}
                  {formError && (
                    <div className="text-xs text-rose-400 bg-rose-900/10 border border-rose-700/20 p-3 rounded-md">
                      {formError}
                    </div>
                  )}
